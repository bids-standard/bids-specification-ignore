...
def make_glossary_myst(schema):
    ...
    text = "# Appendix XIV: Glossary of schema objects\n\n"
    text += "```{glossary}\n\n"
...
def make_glossary(schema):
    ...
    for obj_key in sorted(all_objects.keys()):
        ...
        text += "\n"
        text += f'<a name="{obj_marker}">'
        text += f'<h2>{obj_key}</h2>'  # not ## bc of executablebooks/jupyter-book/issues/1610
        text += "</a>"
        text += f"\n\n"
        text += f"name: {obj_name}\n\n"
        text += f"description:\n>{obj_desc}\n\n"
